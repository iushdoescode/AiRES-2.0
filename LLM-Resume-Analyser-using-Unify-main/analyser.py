...
try:    
    # OpenAI model initialization as an alternative
    model = ChatOpenAI(
        model=st.session_state.endpoint.split('@')[0],  # Extract model name
        openai_api_key=st.session_state.unify_api_key,  # Use the same API key
        temperature=model_temperature
    )
except Exception as e:
    st.warning(f"An error occurred: {str(e)}. Please check your API key or model configuration.")
...
def feature_match_function(resume_text, job_offer):
    with st.spinner("Setting up the model..."):
        feature_match_prompt = PromptTemplate(
            ...
            template = """You are an AI assistant designed to analyze resumes against job descriptions. 
            Your task is to evaluate how well the provided resume matches the job offer requirements.

            Follow these steps carefully:

            1. Skills Analysis:
               - List all matching technical skills
               - List all matching soft skills
            ...
            5. Impact & Achievements:
               ...
               - Check for metrics and results
               - Score impact statements (0-100)

            Provide the output in this exact JSON format:
            {
                "analysis": {
                    "skills": {
                        "matching_technical": ["skill1", "skill2"],
                        "matching_soft": ["skill1", "skill2"],
